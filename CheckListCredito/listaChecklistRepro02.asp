...
<input type="hidden" name="idComboRepro" id="idComboRepro" value="2">
<input type="hidden" name="idZonalesRepro" id="idZonalesRepro" value="<%=id_zonal%>">

<%if perfilMain = "3" then%>
  <div class="controls">
    <span class="btn btn-success btnVolverVisZPaisrep">Volver</span>
  </div>
  <p></p>
<%end if%>

<table id="tableP022" class="table table-bordered table-hover table-condensed table-striped mano" data-anio2="<%=anio%>" data-mes2="<%=mes%>" data-zonal="<%=id_zonal%>" data-perfil="<%=perfilMain%>" data-usuario="<%=idUsuarioMain%>">
<thead>
<tr>
...
$(document).ready(function() {  
  var mes = $('#tableP022').attr('data-mes2');
  var anio = $('#tableP022').attr('data-anio2');
  var id_zonal = $('#tableP022').attr('data-zonal');
  var idUsuarioMain = $('#tableP022').attr('data-usuario');
  var perfilMain = $('#tableP022').attr('data-perfil');
  //alert(perfilMain);
  var url = 'CheckListCredito/datosTab002.asp?mes='+mes+'&anio='+anio+'&id_zonal='+id_zonal+'&perfilMain='+perfilMain+'&idUsuarioMain='+idUsuarioMain;  
  $('#tableP022').dataTable( {
    "ajax": url,    
  ...
  $('#tableP022 tbody').delegate('tr','click', function(){
...
$('.btnVolverVisZPaisrep').click(function(){   
    var mes = <%=mes%>;    
    var anio = <%=anio%>;
