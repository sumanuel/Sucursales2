...
<div class="row-fluid">
	...
	'tabla'
		...
		sql = sql & " group by a.fecha) as pagos_prog_qty, "
		sql = sql & " (select SUM(a.valor) "
		sql = sql & " from SUC_index_ips a "
		...
		sql = sql & " and a.fecha = z.fecha group by a.fecha) as pagos_real_qty, "
		sql = sql & " (select SUM(a.valor) "
		sql = sql & " from SUC_index_ips a "
	        	...
	        	<div class="span12">
			   		...
			   		<thead>
			   			<tr>
							...
							<th class="bajaLetra">BP Prg QTY</th>
							<th class="bajaLetra">BP Prg $</th>
							<th class="bajaLetra">BP Real QTY</th>
							<th class="bajaLetra">BP Real $</th>
			   			</tr>
			   		</thead>
			   		<tbody>
						<%For i = 0 to ubound(datos, 2)
							...
							<tr id="trRegion<%=trim(datos(0,i))%>" class="oculto">
								<td colspan="9" id="tdRegion<%=trim(datos(0,i))%>"></td>
							</tr>
						<%next%>
					</tbody>
				</table>
