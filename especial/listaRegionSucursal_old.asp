		...
		if not rs.eof then
			...
			<table class="table table-bordered table-hover">
				...
				<tbody>
					...
					<tr>
						<td class="nombreSucursal" data-nombreSucursal="<%=server.htmlEncode(trim(datos(3,i)))%>">
						...
						<td><%=formatnumber(trim(datos(8,i)),0)%></td>
						<td>$<%=formatnumber(trim(datos(9,i)),0)%></td>
						<td><%=formatnumber(trim(datos(10,i)),0)%></td>
						<td>$<%=formatnumber(trim(datos(11,i)),0)%></td>
					</tr>
						<%serie = serie & "['"&trim(datos(3,i))&"',"&trim(datos(6,i))&"],"%>
					<%next
					largoSerie = len(serie)-1
					serie = left(serie,largoSerie)
					serie = serie & "]"%>
				</tbody>
			</table>
		<%end if%>
...
<script type="text/javascript">
	$(function () {
		...
		var serieValorRealSucursal = [];
		$('.nombreSucursal').each(function(){
			valoresCategorias.push($(this).attr('data-nombreSucursal'));
		});
		$('.serieValorRealSucursal').each(function(){
			serieValorRealSucursal.push($(this).attr('data-serieValorRealSucursal'));
		});
