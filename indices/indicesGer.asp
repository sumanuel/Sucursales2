...
<div class="row-fluid">
  <%if perfil = "3" then%>
    <div id="volverIndiceGerencia" class="offset10 alert alert-error mano text-center">
      Volver a vista zonal 
      <i class="icon-reply"></i>
    </div>
  <%end if%>
  <%if perfil = "2" then%>
    <div id="volverIndiceZonal" class="offset10 alert alert-error mano text-center">
      Volver a vista zonal 
      <i class="icon-reply"></i>
    </div>
  <%end if%>
</div>
<div id="muestraGraficoIndiceGer" class="well oculto">
</div>
...
if not rs.eof then
  do while not rs.eof
    ...
    %>
  <div id="indice<%=i%>" data-idIndice="<%=idIndice%>" class="indice span4 well">
      ...
      do while not rs2.eof
        ...
        idItemIndice = trim(rs2("idIndice"))
          ...
          <tr>
            ...
            <td width="40%" align="right" valign="middle"><div id="v<%=area%><%=tipo%>"><% if tipo<>0 then response.write("0") end if %></div></td>
          </tr>
...
$('#volverIndiceGerencia').click(function(){
  $('#areaTrabajoGerencia').fadeOut('fast');
  $('#menuGer, #datosZona').fadeIn('slow');
});

$('#volverIndiceZonal').click(function(){
  location.reload();
});
$('.abreGrafico').click(function(){
  ...
  var div = 'muestraGraficoIndiceGer'
  var datos = 'idIndice='+idIndice;
  setTimeout(function(){
  try{
...
function renderData(objectTable){
  ...
  $.each(objectTable, function(index, value) {            
    ...
    if($(objIndex)){
      var valor = numberFormat(value.valor);
