...
if not rs.eof then
 ...
 '###########################################################################################################'
 '' 		sql2 = " "
''		sql2 = sql2 & " select count(id_asist_personal) as asistencia "
''		sql2 = sql2 & " from SUC_sucursal_asistencia_personal  "
''		sql2 = sql2 & " where (rut_personal in "
''		sql2 = sql2 & " (select rut_personal "
''		sql2 = sql2 & " from SUC_sucursal_personal "
''		sql2 = sql2 & " where id_cargo=1) or  rut_personal "
''		sql2 = sql2 & " in (select rut_reemp "
''		sql2 = sql2 & " from SUC_sucursal_reemplazos "
''		sql2 = sql2 & " where id_cargo in (1,6)))  "
''		sql2 = sql2 & " and id_sucursal = '"&idSucursal&"' "
''		sql2 = sql2 & " group by asistencia "
''		
''		'Response.Write(sql2)
''		'Response.End()
''		set rs2 = db.execute(sql2)
...
''			datos2 = rs2.GetRows()
''			For x = 0 to ubound(datos2, 2)
''				totalCajerosSucursal = formatnumber(trim(datos2(0,x)),2)
''			next
''		else
''			totalCajerosSucursal = 0
''		end if
''		if totalCajerosSucursal <> 0 then
''			sql2 = ""
''			sql2 = sql2 & " select count(id_asist_personal) as asistencia "
''			sql2 = sql2 & " from SUC_sucursal_asistencia_personal  "
''			sql2 = sql2 & " where (rut_personal in "
...
''			sql2 = sql2 & " where id_cargo in (1,6)))  "
''			sql2 = sql2 & " and id_sucursal = '"&idSucursal&"' "
''			sql2 = sql2 & " and asistencia is NULL "
''			sql2 = sql2 & " group by asistencia"
''			set rs2 = db.execute(sql2)
''			if not rs2.eof then
''				datos2 = rs2.GetRows()
''				For x = 0 to ubound(datos2, 2)
''					totalCajerosAusentesSucursal = formatnumber(trim(datos2(0,x)),2)
''				next
''			else
''				totalCajerosAusentesSucursal = 0
''			end if
''		else
''			totalCajerosAusentesSucursal = 0
''		end if
''
''		totalCajerospresentesSucursal = totalCajerosSucursal - totalCajerosAusentesSucursal
''		if totalCajerospresentesSucursal <> 0 then
''			porcentajeCajerosSucursal = 100 - totalCajerospresentesSucursal
''			porcentajeCajerosSucursal = porcentajeCajerosSucursal / totalCajerosSucursal
''		else
''			porcentajeCajerosSucursal = 0
''		end if
''		
''		Response.Write "   ""porcentajeCajerosSucursal"": """ & formatnumber(porcentajeCajerosSucursal,2) & """, "
''
''
		sql2 = ""
		...
		sql2 = sql2 & " from(select a.id_zonal, "
