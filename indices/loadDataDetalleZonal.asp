...
<%
idZona = trim(request("idZona"))
sql = ""
sql = sql & " select id_usuario, id_zona "
...
if not rs.eof then
	...
	For i = 0 to ubound(datos, 2)
		...
		sql2 = sql2 & " and a.id_sucursal in (select id_sucursal from SUC_zonales_sucursal "
		sql2 = sql2 & " where id_zonal = '"&idZonal&"') "
...
'################################################################################
		...
		sql2 = sql2 & " from SUC_zonales_sucursal "
		...
		sql2 = sql2 & " from SUC_zonales a "
		sql2 = sql2 & " inner join SUC_zonales_sucursal b "
		...
		sql2 = sql2 & " group by z.id_zonal, z.id_usuario "
		set rs2 = db.execute(sql2)
		if not rs2.eof then
			datos2 = rs2.GetRows()
...
'##########################################################################
	...
	sql2 = sql2 & " SUC_zonales_sucursal where "
...
'##########################################################################
		...
		sql2 = ""
		sql2 = sql2 & " select "
		sql2 = sql2 & " w.id_usuario, "
		sql2 = sql2 & " w.zonal, "
		sql2 = sql2 & " (cast((w.total_apertura*100) as float)/CAST(w.total as float)) as porc_apertura, "
		sql2 = sql2 & " (cast(((w.total-w.total_apertura)*100) as float)/CAST(w.total as float)) as porc_sinr, "
		sql2 = sql2 & " (cast((w.total_cerrada*100) as float)/CAST(w.total as float)) as porc_cerradas "
		sql2 = sql2 & " from ( "
		sql2 = sql2 & " select "
		sql2 = sql2 & " z.id_usuario, "
		sql2 = sql2 & " z.zonal, "
		sql2 = sql2 & " (select COUNT(*) from SUC_zonales_sucursal where id_zonal = z.id_usuario) as total, "
		sql2 = sql2 & " (select COUNT(*) from SUC_sucursal_apertura "
		sql2 = sql2 & " where fecha_ingreso = cast(GETDATE() as DATE) and tipo = 1 and "
		sql2 = sql2 & " id_sucursal in (select id_sucursal from SUC_zonales_sucursal "
		sql2 = sql2 & " where id_zonal = z.id_usuario)) as total_apertura, "
		sql2 = sql2 & " (select COUNT(*) from SUC_sucursal_apertura "
		sql2 = sql2 & " where fecha_ingreso = cast(GETDATE() as DATE) and tipo = 2 and "
		sql2 = sql2 & " id_sucursal in (select id_sucursal from SUC_zonales_sucursal "
		sql2 = sql2 & " where id_zonal = z.id_usuario)) as total_cerrada "
		sql2 = sql2 & " from ( "
		sql2 = sql2 & " select id_usuario, zonal from SUC_zonales where id_usuario = '"&idZonal&"' "
		sql2 = sql2 & " ) as z ) as w "
		set rs2 = db.execute(sql2)
		if not rs2.eof then
