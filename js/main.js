...
$("#tabMenu13CajerosE").click(function () {
  sube();
  $("#gastosIndices").removeClass("oculto");
...
$(".botonCambiaEstado").click(function () {
  ...
  $("." + idGestion).html(
    '<select class="selectCambiaEstadoSucursal input-small ' +
      idGestion +
      '" data-idGestion=' +
      idGestion +
      ' id="selectCambiaEstadoSucursal"><option value="">[Seleccione opcion]</option><option value="1">Ingresado por error</option><option value="2">Cerrar</option></select><div id="validaOpcion" class="' +
      idGestion +
      '"></div>',
  );
});
$(".selectCambiaEstadoSucursal").change(function () {
  ...
  if (valorSeleccionado === "") {
    ...
    $("#validaOpcion." + idGestion)
      ...
      .html(
        '<span class="btn btn-mini btn-success" id="botonGuardaEstadoGestion" data-idGestion="' +
          idGestion +
          '" data-valorSeleccionado="' +
          valorSeleccionado +
          '"><strong><i class="icon-save"></i> Guardar</strong></span><span class="btn btn-mini btn-danger" id="botonCancelaEstadoGestion" data-idGestion=' +
          idGestion +
          '><i class="icon-remove"></i> Cerrar</span>',
      );
  }
});
...
$("#botonCancelaEstadoGestion").click(function () {
  var idGestion = $(this).attr("data-idGestion");
  $("#divCambiaEstadoGestion." + idGestion).html(
    '<span class="btn btn-mini btn-success botonCambiaEstado" data-idGestion="' +
      idGestion +
...
function sube() {
  try {
    $(
      "#areaInformes,#areaTrabajoGer,#areaDashGer,#areaTransacciones,#areaGastosGer,#sucursalesZonal,#areaTrabajoZonal,#areaJeps,#miSucursal,#panelSucursal,#areaInformes,#gastosZonal,#indicesZOnal, #gastosIndices, #personalGer, #checkListGer, #reporteGer, #areaGastosGer, #gastosGerEspacio, #divDetalleSucursales, #areaDivTabsGerencia",
    ).slideUp("fast");
  } catch (er) {}
}
    ...
    if (perfil == "2") {
      ...
      setInterval(function () {
        if ($("#tabMenu1").hasClass("active")) {
          ...
          var totalRecarga = 0;
          if (
            totalMinutosActuales >= 0 &&
            totalMinutosActuales <= 480
          ) //0 -> 8 am
          {
            totalRecarga = 3600000;
          }
          if (
            totalMinutosActuales >= 481 &&
            totalMinutosActuales <= 600
          ) // 8:01 -> 10 am
          {
            totalRecarga = 30000;
          }
          if (
            totalMinutosActuales >= 601 &&
            totalMinutosActuales <= 1050
          ) // 10:10 -> 17:10
          {
            totalRecarga = 3600000; //3600000
          }
          if (
            totalMinutosActuales >= 1051 &&
            totalMinutosActuales <= 1170
          ) // 1731 -> 19:30
          {
            totalRecarga = 30000;
          }
          if (
            totalMinutosActuales >= 1171 &&
            totalMinutosActuales <= 1439
          ) // 19:30 -> 23:59
          {
            totalRecarga = 3600000;
          }
