...
'			end if 
			...
			hora_entrada=cleanseData(arrRows(6),"time",8)
			if datoError then
				errLog =  errLog & " {""m"": ""Error en formato (Hora Entrada), linea "&contadorLineas&", "&arrRows(6)&"""}, "
				'Response.Write("Error en formato, linea "&contadorLineas&", '"&arrRows(9)&"'")
				'Response.End()
			end if
			
			hora_salida=cleanseData(arrRows(7),"time",8)
			if datoError then
				errLog =  errLog & " {""m"": ""Error en formato (Hora Salida), linea "&contadorLineas&", "&arrRows(7)&"""}, "
