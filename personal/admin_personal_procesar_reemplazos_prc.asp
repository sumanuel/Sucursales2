...
'if Session("id_usuario")<>"" and Session("tipo")="admin" then
	...
	Else
		...
		' --- NUEVA LÓGICA DE CARGA DE PLANIFICACIÓN CORREGIDA ---

		' 1. Truncar la tabla de planificación
		truncatePlanificacion="truncate table dbo.SUC_sucursal_reemplazos_plan"
		DB.Execute(truncatePlanificacion)
		
		' 2. Insertar los datos validados desde la tabla temporal a la tabla de planificación
		' CORRECCIÓN: No incluir id_reemplazo_plan ya que la tabla temporal no lo tiene
		planificacion="insert into dbo.SUC_sucursal_reemplazos_plan(bt_sucursal,id_cargo,nombre_sucursal,nombre_cargo,rut_titular,rut_reemp,nombre_reemp,desde,hasta,hora_ingreso,hora_salida,empresa,motivo,fecha_reg,hora_reg) "
		planificacion=planificacion+"select bt_sucursal,id_cargo,nombre_sucursal,nombre_cargo,rut_titular,rut_reemp,nombre_reemp,desde,hasta,hora_ingreso,hora_salida,empresa,motivo,fecha_reg,hora_reg from SUC_sucursal_reemplazos_temp"
		DB.execute(planificacion)

		' 3. Actualizar el nombre del titular en la tabla de planificación
		sql_update_plan = ""
		sql_update_plan = sql_update_plan + "update SUC_sucursal_reemplazos_plan set "
		sql_update_plan = sql_update_plan + "nombre_titular = (rtrim(ltrim(b.nombre_personal)) + ' ' + rtrim(ltrim(b.apep_personal))) "
		sql_update_plan = sql_update_plan + "from SUC_sucursal_reemplazos_plan A "
		sql_update_plan = sql_update_plan + "inner join SUC_sucursal_personal B on A.rut_titular = B.rut_personal"
		DB.execute(sql_update_plan)

		

		' 4. Registrar en la tabla de Log el éxito de la carga web
		sql_log = "INSERT INTO dbo.SUC_sucursal_reemplazos_plan_log (proceso, tipo, mensaje, registros_afectados, fecha_aplicacion, usuario) " & _
				"SELECT 'WEB_CARGA', 'SUCCESS', 'Carga web exitosa. Datos en tabla de planificación. Serán aplicados por Job Batch.', COUNT(1), getdate(), '" & usuario & "' FROM dbo.SUC_sucursal_reemplazos_plan"
		DB.execute(sql_log)

		' 5. Devolver mensaje de éxito al usuario (formato corregido)
		'Response.Write "{"
		'Response.Write "   ""result"": ""ok"", "
		'Response.Write "   ""msg"": ""Carga de reemplazos planificada correctamente. Se aplicará en el Job Batch Nocturno."" "
		'Response.Write "}"

	' --- FIN NUEVA LÓGICA DE CARGA DE PLANIFICACIÓN ---
