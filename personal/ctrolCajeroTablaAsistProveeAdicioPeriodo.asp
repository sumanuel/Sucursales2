...
<%sql = ""
sql = sql & " EXEC SCSS_prc_cajeros_total_asistencia_adicional_por_proveedor '"&periodo&"','1' "
...
<div>
<table id="tablaTotalAsistProveeAdicioPeriodo" class="table table-bordered table-hover1 table-condensed table-striped cuerpoTabla" data-perfilMain="<%=perfilMain%>" data-idUsuario="<%=idUsuario%>" data-periodo="<%=periodo%>">
	<thead>
		<tr>
			<th>Proveedor</th>
			<th>Solicitados</th>
	...
	<%if tieneDatos = 1 then
		for i = 0 to ubound(datos,2)
			...
			if idProveedor = 0 then 
				...
				<tr class="tablaTotalAsistProveeAdicioPeriod1 mano" data-idProveedor="<%=idProveedor%>"  data-periodo="<%=periodo%>">
					<td><%=proveedor%></td>
					<td><%=solicitados%></td>
					<td><%=llegaron%></td>
					<td><%=revisar%></td>
...
<script type="text/javascript">
	$('.tablaTotalAsistProveeAdicioPeriod1').click(function() {
		$('#ctrolCajeroCargaTablaDetalle').html('');
		...
		idUsuario = $('#tablaTotalAsistProveeAdicioPeriodo').attr('data-idUsuario');
		perfilMain = $('#tablaTotalAsistProveeAdicioPeriodo').attr('data-perfilMain');
			
		pagina = 'ctrolCajeroTablaAsistProveeDetalleAdicioPeriodo.asp';
		div = 'ctrolCajeroCargaGrafico';
