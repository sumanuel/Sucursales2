...
sql = ""
sql = sql & " EXEC SCSS_prc_cajeros_asistencia_hoy '2','"&idEmpresa&"','"&positionColumn&"'"
...
if tipoVista = 2 then
	...
	sql = ""
	sql = sql & " EXEC SCSS_prc_cajeros_asistencia_hoy '2','"&idEmpresa&"','"&positionColumn&"','2','"&fechaFormateada&"' "
...
<table id="tablaTotalAsistProveeDetalleDia" class="table table-bordered table-hover1 table-condensed table-striped cuerpoTabla" data-perfilMain="<%=perfilMain%>" data-idUsuario="<%=idUsuario%>" data-tpVista="<%=tipoVista%>" data-fecha="<%=fechaFormateada%>">
	<thead>
		<tr>
			<th>Proveedor</th>
			<th>Sucursal</th>
	...
	<%if tieneDatos = 1 then
		for i = 0 to ubound(datos,2)
			...
			%>
			<tr class="mano" onclick="muestraDetalleTitularMod('<%=idSucursal%>')">
				<td ><%=proveedor%></td>
				<td><%=sucNombre%></td>
				<td><%=rutTitular%></td>
				<td><%=nomTitular%></td>
...
<script type="text/javascript">
	$('#tablaTotalAsistProveeDetalleDia').dataTable( {
		"sDom": "<'row-flid'<'span6'l><'span6'f>r>t<'row-fluid'<'span6'i><'span6'p>>",
		"sPaginationType": "bootstrap",
		"sLoadingRecords": "Cargando...",
	});

	function muestraDetalleTitularMod(idSucursal){
		var pagina, div, datos, idSucursal, periodo, tpVista, fecha;
		periodo = $(this).attr('data-periodo');
		tpVista = $('#tablaTotalAsistProveeDetalleDia').attr('data-tpVista');
		fecha = $('#tablaTotalAsistProveeDetalleDia').attr('data-fecha');
		idUsuario = $('#tablaTotalAsistProveeDetalleDia').attr('data-idUsuario');
		perfilMain = $('#tablaTotalAsistProveeDetalleDia').attr('data-perfilMain');
 		
		//no sufre modificación para traspaso
