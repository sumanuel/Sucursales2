...
sql = ""
sql = sql & " EXEC SCSS_prc_cajeros_total_asistencia_por_proveedor '"&periodo&"','2' "
'response.write(sql)
'response.end
tieneDatos = 0
...
<table id="tablaTotalAsistenciaProveedorDetallePeriodo" class="table table-bordered table-hover2 table-condensed table-striped cuerpoTabla table-hover2-cells" data-perfilMain="<%=perfilMain%>" data-idUsuario="<%=idUsuario%>" data-periodo="<%=periodo%>">
	<thead>
		<tr>
	...
	<%if tieneDatos = 1 then
		for i = 0 to ubound(datos,2)
			...
			%>
			<tr class="muestraDetalleAsistenProveed">
				<td style="background-color: #f2f2f2; text-align: center;" class="nombreSerie" data-valor="<%=fecha%>"><%=fecha%></td>
				<td class="valorSerie1 mano" data-valor="<%=solicitadoECR%>" onclick="muestraDetalleTitular2(this,'1','<%=fecha%>')"><%=solicitadoECR%></td>
				<td class="valorSerie2 mano" data-valor="<%=llegaronECR%>" onclick="muestraDetalleTitular2(this,'1','<%=fecha%>')"><%=llegaronECR%></td>
				<td class="valorSerie3 mano" data-valor="<%=revisarECR%>" onclick="muestraDetalleTitular2(this,'1','<%=fecha%>')"><%=revisarECR%></td>
				<td style="background-color: #e6e6ff; text-align: center;" ><%=ECR%></td>
				<td class="mano" onclick="muestraDetalleTitular2(this,'2','<%=fecha%>')"><%=solicitadoSINE%></td>
				<td class="mano" onclick="muestraDetalleTitular2(this,'2','<%=fecha%>')"><%=llegaronSINE%></td>
				<td class="mano" onclick="muestraDetalleTitular2(this,'2','<%=fecha%>')"><%=revisarSINE%></td>
...
<script type="text/javascript">
	$(function(){
		...
		$('#tablaTotalAsistenciaProveedorDetallePeriodo').dataTable( {
	...
	function muestraDetalleTitular2(positionColumn, idEmpresa, fecha) {
		var idEmpresa = idEmpresa;	
        var positionColumn =  positionColumn.cellIndex;
        var fecha = fecha;
    	...
    	idUsuario = $('#tablaTotalAsistenciaProveedorDetallePeriodo').attr('data-idUsuario');
		perfilMain = $('#tablaTotalAsistenciaProveedorDetallePeriodo').attr('data-perfilMain');
    	pagina = 'ctrolCajeroTablaAsistProveeDetalleHoy.asp';
...
function creaGrafico(){
		var rowCount = $('#tablaTotalAsistenciaProveedorDetallePeriodo tr').length -1;
	...
	$('#ctrolCajeroCargaGrafico').highcharts({
	      chart: {
		        type: 'column',
				width: 600,
				height: 410,
