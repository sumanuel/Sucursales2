...
sql = ""
sql = sql & " EXEC SCSS_prc_cajeros_asistencia_hoy '1' "
'response.write(sql)
'response.end
tieneDatos = 0
...
<div>
<table id="tablaTotalAsistenciaProveedorHoy" class="table table-bordered table-hover2 table-condensed table-striped cuerpoTabla table-hover2-cells" data-perfilMain="<%=perfilMain%>" data-idUsuario="<%=idUsuario%>" data-periodo="<%=periodo%>">
	<thead>
		<tr>
			<th>Proveedor</th>
			<th>Solicitados</th>
	...
	<%if tieneDatos = 1 then
		for i = 0 to ubound(datos,2)
			...
			<%
				...
				<tr>
					<th><%=proveedor%></th>
					<td class="mano" onclick="muestraDetalleTitular1(this,'<%=idEmpresa%>')"><%=solicitados%></td>
					<td class="mano" onclick="muestraDetalleTitular1(this,'<%=idEmpresa%>')"><%=llegaron%></td>
					<td class="mano" onclick="muestraDetalleTitular1(this,'<%=idEmpresa%>')"><%=revisar%></td>
					<th><%=cobertura%></th>
				</tr>
			<%end if	
...
<script type="text/javascript">
	function muestraDetalleTitular1(positionColumn, idEmpresa) {
    	...
    	idUsuario = $('#tablaTotalAsistenciaProveedorHoy').attr('data-idUsuario');
		perfilMain = $('#tablaTotalAsistenciaProveedorHoy').attr('data-perfilMain');
