...
if tipoPrc = "1" then 'OBTIENE DATA DEL GUARDIA TITULAR
	...
	sql = sql & "guardia_p_entrada, "
	sql = sql & "guardia_p_salida "
	sql = sql & "from SUC_sucursal_guardias_t "
	sql = sql & "where guardia_rut = '"& rutPersonal &"'"
	...
	if not rs.eof then 
		 ...
		 tipo = trim(rs("tipo"))
		 sucursal = trim(rs("sucursal"))
		 guardia_nombre = trim(rs("guardia_nombre"))
		 guardia_p_entrada = trim(rs("guardia_p_entrada"))
		 guardia_p_salida = trim(rs("guardia_p_salida"))		 
		 empresa = trim(rs("empresa"))
		 
		 Response.Write "   ""cod_bantotal"": """&cod_bantotal&""", "
		 Response.Write "   ""sucursal"": """&sucursal&""", "
		 Response.Write "   ""tipo"": """&tipo&""", "
		 Response.Write "   ""guardia_nombre"": """&guardia_nombre&""", "
		 Response.Write "   ""guardia_p_entrada"": """&guardia_p_entrada&""", "
		 Response.Write "   ""guardia_p_salida"": """&guardia_p_salida&""", "		 
