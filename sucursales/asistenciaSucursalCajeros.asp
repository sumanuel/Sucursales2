...
<div class="row-fluid">
  <span class="span12 alert alert-success">
    ...
    <!--<span class="span4">
      <span class="btn btn-primary addCaso"><i class="icon-book"></i><span style="font-size: 10px"><strong> AGREGAR CASO</strong></span></span>
      <span class="btn btn-warning verCasos"><i class="icon-list"></i><span style="font-size: 10px"><strong> VER CASOS</strong></span></span>
    </span>-->
  </span>  
</div>

<div class="row-fluid oculto" id="frmAddCaso">
  <div id="divAddCaso">
      <span class="span12">
        ...
        <form class="form-horizontal">
            ...
            <div class="control-group oculto" id="cajerosRelacionadosHead">
              ...
              <div class="controls">
                ...
                <div class="oculto" id="cajerosSeleccionado">
            ...
            <div class="control-group">
              <div class="controls">          
                ...
                <a class="btn btn-danger addCasoCerrar"><span style="font-size: 10px"><strong>CERRAR</strong></span></a>
              </div>
            </div>
...
<div class="row-fluid" id="frmAsistencia">
  ...
  <table border="0" class="table table-bordered table-hover" align="center">
      <tr bgcolor="" align="center">
          ...
          <td class="" width="7%"><strong><span class="ayuda" data-placement="top" title data-original-title="Ingreso Informado">Informado</span></strong></td>
          ...
          <td class="" width="7%"><strong>---</strong></td>
      </tr>
      <%
  	arr_idasist = ""
  	do while not rs.eof 
      ...
      <tr align="center">
          ...
          <td align="center">
          <%if asistencia = "" then%> 
            <b><%Response.Write("S/R")%></b>
          <%else%>
            <b><%Response.Write("SI")%></b>
          <%end if%>
          </td>
...
<% else 'ELSE: se ejecuta cuando sucursal no tiene cajeros u otro cargo en la asistencia %>
<div class="row-fluid span12 well">
<div class="row-fluid">
  <span class="span12 alert alert-success">
    <span class="span8">
...
<div class="row-fluid" id="frmAsistencia">
  <input type="hidden" name="v_idSucursal" id="v_idSucursal" value="<%=idSucursal%>"/>
  <input type="hidden" name="idUsuario" id="idUsuario" value="<%=idUsuario%>"/>
	<span class="span11 alert alert-danger">
		<strong>
        	<h5>Tips:</h5>
            <h6>
            	<ol>
...
<% end if %>
 ...
 $('.addCaso').click(function(){    
    $('#frmAsistencia').slideUp();
    $('#divCasoDetalle').slideUp();

    $('#frmAddCaso').removeClass('oculto');
    $('#frmAddCaso').slideDown();
    $('#divAddCaso').slideDown();
    $('#divAddCasoSucces').slideUp();

    $('#divAddCasoSucces').removeClass('listCasosOpen');    
 });

 $('.addCasoCerrar').click(function(){
    $('#frmAddCaso').addClass('oculto');    
    cleanAddCasoForm();
    $('#frmAsistencia').slideDown();
 });

 $('.verCasos').click(function(){
    //alert('ver casos');    

    if($('#divAddCasoSucces').hasClass('listCasosOpen')){
        $('#divAddCasoSucces').slideUp();
        $('#divAddCasoSucces').removeClass('listCasosOpen');
        $('#frmAsistencia').slideDown();
        $(this).html('<i class="icon-list"></i> <span style="font-size: 10px"><strong>VER CASOS</strong></span>').removeClass('btn-danger').addClass('btn-warning');
    }
    else{
      $('#frmAsistencia').slideUp();   
      $('#divAddCasoSucces').addClass('listCasosOpen');
      $('#divCasoDetalle').slideUp();
      $(this).html('<i class="icon-remove-sign"></i> <span style="font-size: 10px"><strong>CERRAR LISTADO CASOS</strong></span>').removeClass('btn-warning').addClass('btn-danger');
      var idSucursalSistema = $('#v_idSucursal').val();

      var pagina = 'sucursales/atencionCliente_sql.asp';
      var datos = 'tipo=4&idSucursalSistema='+idSucursalSistema;
      ...
      $.ajax({
          ...
          success: function(source){    
            $('#divAddCasoSucces').html(source);
            $('#divAddCaso').slideUp();
            $('#divAddCaso').addClass('oculto');
            $('#frmAddCaso').removeClass('oculto');
 ...
 $('.casoMotivo').change(function(){ 
    ...
    //alert('data_flagreqcajero:'+data_flagreqcajero);
    if(data_flagreqcajero == '1'){
        var pagina = 'sucursales/atencionCliente_sql.asp';
