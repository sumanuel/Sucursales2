        ...
        <div class="span12">
            ...
            <script type="text/javascript">
                ...
                $('.btnEliminarSolicitud').click(function(){
                    ...
                    if(confirm('ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿EstÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡ seguro que desea eliminar la solicitud de ' + motivo + '?')){
                        $.ajax({
                            type: 'POST',
            ...
            function cargarTablaSolicitudesPaginada(pagina) {
                ...
                $.ajax({
                    url: '/sucursales2/sucursales/obtener_solicitudes_paginadas.asp',
                    type: 'GET',
                    data: params,
                    dataType: 'json',
                    success: function(data) {
...
function registrarLog(funcionalidad, tipoAccion) {
    $.ajax({
        url: '/sucursales2/personal/registrar_log.asp',
        data: {
...
function cargarTablaSolicitudes() {
    ...
    console.log('ParÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡metros:', params);
        ...
        success: function(solicitudes) {
            ...
            if(solicitudes.length === 0) {
                ...
                $.each(solicitudes, function(i, sol){
                    ...
                    tr += '<td align="center">';
                    <% If perfil = "3" Then %>
                    // Perfil 3 (CENTRAL) solo puede cambiar estado si está en APROBADA ZONAL (3)
                    if(parseInt(sol.id_estado) === 3) {
                        tr += '<i class="icon-exchange icon-large mano btnCambiarEstado" data-id="' + sol.id_solicitud + '" data-estado="' + sol.id_estado + '" title="Cambiar Estado"></i> ';
                    }
                    <% Else %>
                    ...
                    <% ElseIf perfil = "3" Then %>
                    // Perfil 3 (CENTRAL) solo puede editar si está en APROBADA ZONAL (3)
                    if(parseInt(sol.id_estado) === 3) {
                        tr += '<i class="icon-pencil icon-large mano btnEditarSolicitud" data-id="' + sol.id_solicitud + '" title="Editar"></i> ';
                    }
                    <% End If %>
                    // Perfil 2 (ZONAL) no tiene botÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n de editar
                    tr += '<i class="icon-list-alt icon-large mano btnVerHistorial" data-id="' + sol.id_solicitud + '" title="Ver Historial"></i>';
                    tr += '</td>';
                    tr += '</tr>';
                    tbody.append(tr);
...
function activarEventosBotones() {
    ...
    $('.btnEliminarSolicitud').off('click').on('click', function(){
        var id = $(this).data('id');
        if(confirm('ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿EstÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡ seguro de eliminar esta solicitud?')){
            $.ajax({
                type: 'POST',
                url: '/sucursales2/sucursales/eliminar_solicitud_cajero.asp',
                data: {id: id},
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(jsonStart >= 0 && jsonEnd > 0){
                    ...
                    console.log('JSON extraÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­do y limpio:', jsonStr);
                    ...
                    console.error('No se encontrÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³ JSON vÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡lido en la respuesta');
                    alert('Respuesta invÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡lida del servidor');
                }
            },
            error: function(xhr, status, error){
                console.error('Error AJAX:', status, error);
    ...
    <div class="modal-body">
        <form id="formCambiarEstado">
            ...
            <div class="control-group" id="divComentarioEstado">
                ...
                <div class="controls">
                    <textarea id="txt_comentario_estado" name="txt_comentario_estado" class="span10" rows="3" placeholder="Ingrese un comentario sobre el cambio de estado" required></textarea>
                </div>
            </div>
        </form>
    </div>
